...
[~, posVocab, ~] = intersect(vocab, stoplist);
X_train = X_train(~ismember(X_train(:,2), posVocab), :);
X_test = X_test(~ismember(X_test(:,2), posVocab), :);
...
% (f)
fprintf('(f)\n\n');
fprintf('Size of new dictionary: %d\n\n', nwords_all);
fprintf('     calculating betas... (~15s)\n\n');
...
% average test document length - look for better way
total = 0;
docnum = 0;
for n=1:length(Y_test) % 7505 iterations (slow)
    document = X_test(X_test(:,1)==n,:);  
    if isempty(document)
        disp('');
    else
        docnum = docnum + 1;
        total = total + sum(document(:,3));
    end
end
total / docnum
%}
